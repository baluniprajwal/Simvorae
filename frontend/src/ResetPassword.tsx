    ...
    <div className="min-h-screen bg-[#fcfbf9] px-5 py-8 text-[#1a1a1a] font-sans">
      <main className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-5xl gap-10 lg:grid-cols-[0.9fr_1fr] lg:items-center">
        <section>
          ...
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.34em] text-stone-400">Secure Reset</p>
          ...
          <p className="mt-7 max-w-md text-[15px] font-light leading-7 text-stone-600">
            Choose a password with at least 8 characters. Reset links expire after 30 minutes.
          </p>
        </section>
        ...
        <section className="rounded-[1.6rem] border border-stone-200 bg-white p-6 shadow-[0_16px_50px_rgba(0,0,0,0.06)] md:p-8">
          <div className="mb-8 flex items-center gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-[0.24em] text-stone-400">Password Reset</p>
              <h2 className="font-serif text-2xl">Update password</h2>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-center text-xs text-stone-500">
              {message ? (
                <Link to="/login" className="font-bold uppercase tracking-[0.18em] text-[#111] hover:text-stone-500">Sign In</Link>
              ) : (
